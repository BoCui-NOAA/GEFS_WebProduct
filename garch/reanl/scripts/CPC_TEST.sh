
if [ $# -lt 2 ]; then
   echo "Usage:$0 need input"
   echo "1). YYYYMMDDHH (initial time)"
   echo "2). FHR (forecast hours)     "
   exit 8
fi

tmpdir=/ptmp/wx20yz/canomaly
mkdir $tmpdir
cd    $tmpdir

nhoursx=/nwprod/util/exec/ndate

fdir=/nbns/global/wx20yz/CDAS
CDATE=$1             
fhr=$2
YMD=`echo $CDATE | cut -c1-8`
CYC=`echo $CDATE | cut -c9-10`

echo " ***************************************"
echo " JOB INPUT INITIAL  TIME IS: $CDATE "
echo " JOB INPUT FORECAST TIME IS: $fhr   "
echo " ***************************************"

for FHR in $fhr 
do

 FDATE=`$nhoursx +$FHR $CDATE`
 HH=`echo $FDATE | cut -c9-10`
 MDH=`echo $FDATE | cut -c5-10`         
 MD=`echo $FDATE | cut -c5-8`         
 IDATE=`echo $CDATE | cut -c3-10`

 #for ens in p01 p02 p03 p04 p05 p06 p07 p08 p09 p10 p11 p12 p13 p14
 for ens in p01 
 do

 cp /com/gens/prod/gefs.$YMD/00/pgrba/ge${ens}.t${CYC}z.pgrbaf${FHR} fcst_$ens.dat
 cp /com/gens/prod/gefs.$YMD/00/pgrba_bc/ge${ens}.t${CYC}z.pgrba_bcf${FHR} bias_$ens.dat
 cp /com/gens/prod/gefs.$YMD/00/pgrba_an/ge${ens}.t${CYC}z.pgrba_anf${FHR} anom_$ens.dat
set -x
 cp $fdir/cmean_1d.1959$MD mean_$ens.dat
 cp $fdir/cstdv_1d.1959$MD stdv_$ens.dat
set +x

 echo "&namin " >input_$ens
 echo "cfcst='fcst_$ens.dat'," >>input_$ens
 echo "cmean='mean_$ens.dat'," >>input_$ens
 echo "cstdv='stdv_$ens.dat'," >>input_$ens
 echo "cbias='bias_$ens.dat'," >>input_$ens
 echo "canom='anom_$ens.dat'," >>input_$ens
 echo "idate=$IDATE,"          >>input_$ens
 echo "ifh=$FHR,"              >>input_$ens
 echo "/" >>input_$ens

 #$HOME/reanl/sorc/climate_CPC_TEST.exe <input_$ens >output_$ens
 $HOME/reanl/sorc/climate_CPC_TEST.exe <input_$ens 

 done

done

