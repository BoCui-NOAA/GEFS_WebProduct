...
C--------+---------+---------+---------+---------+----------+---------+--
      ...
      double precision dfcst,fmon(2),opara(2)
c     dimension fmon(2),opara(2)
      logical*1 lb(ixy)
...
c
       ...
       jpds(7) = ilev(ii)
c--------+----------+----------+----------+----------+----------+----------+--
       call getgb(11,0,ixy,jj,jpds,jgds,kf,k,kpds,kgds,lb,fcst,iret)
       if(iret.eq.0) then
        ipds = kpds
        igds = kgds
...
c      jpds(14)= ipds(14)
c--------+----------+----------+----------+----------+----------+----------+--
       call getgb(12,0,ixy,jj,jpds,jgds,kf,k,kpds,kgds,lb,cavg,iret)
       if(iret.eq.0) then
...
c      jpds(14)= ipds(14)
c--------+----------+----------+----------+----------+----------+----------+--
       call getgb(13,0,ixy,jj,jpds,jgds,kf,k,kpds,kgds,lb,stdv,iret)
       if(iret.eq.0) then
...
ccc     protect when stdv = 0.0
        if (fmon(2).eq.0.0) fmon(2) = 0.1
        call pelnor(fmon,opara)
        anom(ij)=cdfnor(dfcst,opara)*100.0
c       anom(ij)=cdfnor(fcst(ij),opara)*100.0
        if (ij.ge.10000.and.ij.le.10099) then
        print *, 'ij=',ij,' fm1=',fmon(1),' fm2=',fmon(2),
     *           ' fc=',fcst(ij),' an=',anom(ij)
        endif
       enddo
       call putgb(81,ixy,ipds,igds,lb,anom,iret)
