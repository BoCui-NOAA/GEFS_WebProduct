...
!  READ IN US REGIONAL MASK FILE                   
      ...
      write  (6,*) 'RFC REGIONAL MASK: ',cmask(1:80) 
      lcmask=len_trim(cmask)

!  READ IN DOWNSCALING RATIO FILE                   
      read   (5,80,end=9000) cdsr
      write  (6,*) 'DOWNSCALING RATIO: ',cdsr(1:80)
      ldsr=len_trim(cdsr)
...
122    format(i3.3)
           ...
           cpgbf=ctmpd(1:lctmpd) // '/' //'geprcp.t'// &
                 datcyc(1:2)// 'z.ndgd2p5.bc_24hf' //datcmd//'.gb2'
           copts=ctmpd(1:lctmpd) // '/' //'geprcp.t'// &
                 datcyc(1:2)// 'z.ndgd2p5_conus.24hf' //datcmd//'.gb2'
