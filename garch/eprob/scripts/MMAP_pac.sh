...
sed -e "s/IYMDH/$CDATE/" \
    ...
    -e "s/VYMD31/$VYMD31/" \
    -e "s/VYMD32/$VYMD32/" \
    $SHOME/yan.luo/eprob/scripts/mmap_pac.gs >mmap.gs
...
do
 export CDATE=$CDATE
 #gxgif -r -x 1100 -y 850 -i mmap_${CDATE}_$fhr.gr -o mmap_${CDATE}_${fhr}_pac.gif
 #gxgif -r -x 695 -y 545 -i mmap_${CDATE}_$fhr.gr -o mmap_${CDATE}_${fhr}_pac.gif
 ftpemcrzdm emcrzdm put $RZDMDIR/$CDATE $tmpdir mmap_${CDATE}_${fhr}_pac.png
done
