      program VFPROB
      parameter(ngrid=10512,nens=21)
      parameter(ifprob=0,ictl=0)
      parameter(nep1=21,jctl=ictl)
...
c      2. ictl   = 1:maximun calculation ( 1:max )                 
...
c--------+---------+---------+---------+---------+---------+---------+---------+
      ...
      do 999 nfcst = 1, nstep
       ...
       print *, '++++++++++++++++++++++++++++++++++++'
       call getgrb(fcst,cfilef(1),cfilef(2),kp5,kp7,jdate,ifh,10)
...
cccccc --- Background probability ( usually from previous month average )
        if (ifprob.eq.0) then
         if (ictl.eq.10) then
          ...
          do np = 0, nenso
           fprob(np) = (100.0/nenso)*float(np)
          enddo
         endif
        else
         read (10,1004)(fprob(np),np=0,nenso)
        endif
        lats = latss(ndom)
        late = latee(ndom)
        lons = lonss(ndom)
...
ccc
        ...
        write(51,1023)
        write(51,1005) (fprob(i),i=0,nenso)
...
c--------+---------+---------+---------+---------+---------+---------+---------+
        if (nenso.eq.20.or.ictl.eq.10) then
         print *, '++++++++++++++++++++++++++++++++++++'
