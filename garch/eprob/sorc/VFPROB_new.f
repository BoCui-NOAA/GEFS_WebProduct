      program PROB  
c     parameter(ngrid=10512,nens=14)
c     parameter(ifprob=0,iprob=0,ictl=0)
      parameter(ngrid=NGRID,nens=NENS)
      parameter(ifprob=IFPROB,iprob=IPROB,ictl=ICTL)
...
c
c     Adding bimode calculation
c     ----------------- Yuejian Zhu & Zoltan Toth (09/13/00)
c     
...
c     hr(11)
c     falm(11)
c     ----  dimension for bimode culculation -----
...
c
     ...
     .          'JUL','AUG','SEP','OCT','NOV','DEC'/
      data cbim/'M2  ','M22 ','M222','M3  ','M33 ','M4  ','M5  ','M1  '/
      ...
      data file1/'/global/shared/stat/cdas/500HGT.'/
      data file2/'/global/shared/stat/cdas/500HGT.'/
c--------+---------+---------+---------+---------+---------+---------+---------+
c     OPEN THE FORMAT AND UNFORMATTED DATA SET              
c--------+---------+---------+---------+---------+---------+---------+---------+
      open(unit=51,file='prob.ens',err=1000)
      open(unit=61,file='bimod.ens',err=1000)
...
cccccc --- Background probability ( usually from previous month average )
        if (ifprob.eq.0) then
         ...
         read (10,1004)(fprob(np),np=0,nens)
        endif
        lats = latss(ndom)
        late = latee(ndom)
        lons = lonss(ndom)
...
c--------+---------+---------+---------+---------+---------+---------+---------+
c     BIMODAL CALCULATION - ZOLTAN TOTH (09/11/2000)
c--------+---------+---------+---------+---------+---------+---------+---------+
        bimodg = 0.0
        print *, '++++++++++++++++++++++++++++++++++++'
 ...
 601    continue
        do nlat = lats, late
         ...
         do nlon = lons, lone
          ...
          nmax =   0
          nmin =   0
c         print *,(fcp(ngp,j),j=1,10)
c         print *,(anp(ngp,j),j=1,10)
c       compute derivative
          do nb = 1, 9
           fcpd(nb)=fcp(ng,nb+1)-fcp(ng,nb)
          enddo
c         print *,(fcpd(j), j=1,9)
...
c          print *,(fm(j,1),j=1,10)
c          print *,(fm(j,2),j=1,10)
c          print *,(fm(j,3),j=1,10)
...
c
        ...
        if(ndom.eq.1) write(61,1031)
        if(ndom.eq.2) write(61,1032)
        if(ndom.eq.3) write(61,1033)
        if(ndom.eq.4) write(61,1034)
        if(ndom.eq.5) write(61,1035)
        if(ndom.eq.6) write(61,1036)

        write(61,1041) nmax1,nmax2,nmax22,nmax3,nmax33,nmax4,nmax5
        write(61,1042) cbim(1),(fcpm2(j,1),j=1,3)
        write(61,1042) cbim(1),(fcpm2(j,2),j=1,3)
        write(61,1042) cbim(2),(fcpm22(j,1),j=1,3)
        write(61,1042) cbim(2),(fcpm22(j,2),j=1,3)
        write(61,1042) cbim(3),(fcpm222(j,1),j=1,3)
        write(61,1042) cbim(3),(fcpm222(j,2),j=1,3)
        write(61,1042) cbim(4),(fcpm3(j,1),j=1,5)
        write(61,1042) cbim(4),(fcpm3(j,2),j=1,5)
        write(61,1042) cbim(5),(fcpm33(j,1),j=1,5)
        write(61,1042) cbim(5),(fcpm33(j,2),j=1,5)
        write(61,1042) cbim(6),(fcpm4(j,1),j=1,7)
        write(61,1042) cbim(6),(fcpm4(j,2),j=1,7)
        write(61,1042) cbim(7),(fcpm5(j,1),j=1,9)
        write(61,1042) cbim(7),(fcpm5(j,2),j=1,9)
        write(61,1042) cbim(8),(fcpm1(j,1),j=11,19)
        write(61,1042) cbim(8),(fcpm1(j,2),j=11,19)
        write(61,1043) xdist2, xdist22,xdist222
c       write(61,1042) print *,(bimodg(1,j),j=1,4)
 998   continue
      rewind (nuc)
 999  continue
 1000 continue
...
c--------+---------+---------+---------+---------+---------+---------+---------+
 ...
 1035 format(' -------------Europe------------------')
 1036 format(' ----------     India   --------------')
 1041 format(6x,'M1=',i5,' M2=',i5,' M22=',i5,' M3=',i5,
     .       ' M33=',i5,' M4=',i5,' M5=',i5)
 1042 format(a4,2x,10(f7.0))
 1043 format(6x,3(f7.3))
      stop
      end
c--------+---------+---------+---------+---------+---------+---------+---------+
