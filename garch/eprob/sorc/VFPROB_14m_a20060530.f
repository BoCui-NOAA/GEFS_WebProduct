...
c
      ...
      dimension fnyy(nep1),fnyn(nep1),fnny(nep1),fnnn(nep1)
...
cccccc --- Background probability ( usually from previous month average )
        if (ifprob.eq.0) then
         if (ictl.eq.10) then
          ...
          do np = 0, nens
           fprob(np) = (100.0/float(nens))*float(np)
          enddo
         endif
        else
...
c
	     ...
	     rfcp(np,nb) = rfcp(np,nb)+1.0*wfac
	     if (anp(ng,nb).ne.0.0) then
	      ranp(np,nb) = ranp(np,nb)+1.0*wfac
	     endif
	    endif
           enddo         ! np
          enddo          ! nb
           ...
           do np = 1, nep1  
            ...
            if (fcp(ng,nb).eq.float(npm)) then
             if (anp(ng,nb).eq.1.) then
              fnyy(np) = fnyy(np)+1.0*wfac
             else
              fnyn(np) = fnyn(np)+1.0*wfac
             endif
            endif
           enddo
           if (ictl.eq.1.and.fcp(ng,nb).eq.-1.0) then
            if (anp(ng,nb).eq.1.) then
             fnyy(1) = fnyy(1)+1.0*wfac
            else
             fnyn(1) = fnyn(1)+1.0*wfac
            endif
           endif
          enddo
         enddo          ! nlon
