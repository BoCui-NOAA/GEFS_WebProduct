...
c ----
      seed=kdate(1)
      print *, seed
      call durand(seed,14,randm)
      call dsortx(randm,1,14,indx)
      write(*,'(14i4)') (indx(i),i=1,14)

      do i = 1, 10512
       do j = 1, 10
        fcp(i,j) = fcst(i,indx(j))
       enddo
      enddo
      do i = 1, 10512
       do j = 1, 10
...
c        read (10,1004)(fprob(np),np=0,nens)
         read (10,1005)(fprob(np),np=0,nenso)
        endif
        lats = latss(ndom)
        late = latee(ndom)
        lons = lonss(ndom)
