      program PROB  
c     parameter(ngrid=10512,nens=10)
c     parameter(ifprob=0,iprob=1,ictl=10)
...
c     fcp(ngrid,20) ; forecast numbers at each point in ten climate bins
c     anp(ngrid,20) ; verification for forecast at each point 
c     rfcp
c     ranp
c     sfcp
c     sanp
...
c                  if ictl=10, nens must be 9.
c                = 0:normal
c
      dimension clip(100),clipm1(100),clipm2(100)
      dimension anl(ngrid),fmrf(ngrid),ft62(ngrid),favn(ngrid)
c     dimension fcst(ngrid,14)
      dimension fcst(ngrid,20)
      dimension fcp(ngrid,20),anp(ngrid,20)
      dimension rfcp(nep1,20),ranp(nep1,20),dfcp(ngrid,20)
      dimension sfcp(nep1),sanp(nep1),fp(nep1),fprob(0:nens)
      dimension bsf(21),bsc(21),bs(21)
      dimension nyy(nep1),nyn(nep1),nny(nep1),nnn(nep1)
      dimension hr(nep1),falm(nep1)
      dimension hrc(21),falmc(21)
      dimension latss(6),latee(6),lonss(6),lonee(6),rfp(20)
     ...
     .          'jul','aug','sep','oct','nov','dec'/
      ...
      data file1/'/ptmp2/wd20yz/vrfy/500HGT.'/
      data file2/'/ptmp2/wd20yz/vrfy/500HGT.'/
c--------+---------+---------+---------+---------+---------+---------+---------+
c     UNIT ASSIGNMENT                                         
c--------+---------+---------+---------+---------+---------+---------+---------+
      ...
      call asnunit(nuc1,'-F cos -C ascii -N ibm',ier)
      call asnunit(nuc2,'-F cos -C ascii -N ibm',ier)
c--------+---------+---------+---------+---------+---------+---------+---------+
...
c--------+---------+---------+---------+---------+---------+---------+---------+
      ...
      do 999 nfcst = 1, nstep
       ...
       print *, '++++++++++++++++++++++++++++++++++++'
       call getanl(anl,ndate,filea,kp5,kp6,kp7,icon)
       print *, '++++++++++++++++++++++++++++++++++++'
       print *, '        READ FORECAST DATA'
       print *, '++++++++++++++++++++++++++++++++++++'
       call getfst(fcst,fmrf,ft62,favn,jdate,mdate,ifh,cfilef,icon)
       if ( iprob.eq.1 ) then
       do ng = 1, ngrid
        fcst(ng,1) = fmrf(ng)
       enddo
...
c
	 ...
	 do n = 1, 100
	  clip(n) = clipm1(n)*fac1+clipm2(n)*fac2
       ...
       close (nuc2)
c--------+---------+---------+---------+---------+---------+---------+---------+
c     READ NEW CLIMATOLOGY, CALCULATE FCST & ANL FALLING 20 CLIM BINS
c--------+---------+---------+---------+---------+---------+---------+---------+
       fcp = 0.0
       anp = 0.0
       rewind (nuc)  
...
ccc
	do n = 1, nenso
	 fc = fcst(ng,n)
         if (fc.le.clip(5)) then
	  fcp(ng,1) = fcp(ng,1)+1
	  goto 102
	 endif
         if (fc.gt.clip(95)) then
	  fcp(ng,20) = fcp(ng,20)+1
	  goto 102
	 endif
         do nb = 2, 19
	  ll = (nb-1)*5 
 ...
 102  continue
        enddo        ! n
c
       if(anl(ng).le.clip(5))then
        anp(ng,1) = anp(ng,1)+1
       endif
       if(anl(ng).gt.clip(95))then
        anp(ng,20) = anp(ng,20)+1
       endif
       do nb = 2, 19
        ll = (nb-1)*5 
        lh = nb*5 
...
ccc
          ...
          do nb = 1, 20
           if (nb.ne.nbmax) then
            dfcp(ng,nb) = (100.0-dfcp(ng,nbmax))/19.0
            fcp(ng,nb)  = -1.0
           endif
          enddo
         endif
...
c	
	  write(51,1024)
	  write(51,1002) (bsf(i),i=1,21)
	  write(*, 1002) (bsf(i),i=1,21)
	  write(51,1002) (bsc(i),i=1,21)
	  write(51,1002) (bs(i),i=1,21)
c--------+---------+---------+---------+---------+---------+---------+---------+
	   ...
	   do nlon = lons, lone
	    ...
	    do nb = 1, 20
	     ...
	     rpfc = rpfc+(sfcbc-sanbs)**2
            enddo 
	    rpfs  = 1.0-(1.0/19.0)*rpfs
	    rpfc  = 1.0-(1.0/19.0)*rpfc
...
c--------+---------+---------+---------+---------+---------+---------+---------+
         if ( nens.eq.14.or.nens.eq.10.or.ictl.eq.10 ) then
          print *, '++++++++++++++++++++++++++++++++++++'
...
c
     ...
     .               hrc,falmc,nuc)
	  write(51,1026)
	  write(51,1002) (hrc(nb),nb=1,21)
	  write(*, 1002) (hrc(nb),nb=1,21)
	  write(51,1002) (falmc(nb),nb=1,21)
c
...
c--------+---------+---------+---------+---------+---------+---------+---------+
 ...
 1004 format(15f6.1)
 1005 format(11f6.1)
 1021 format('SCORES  AT VALID TIME ',i8,' (ic : ',i8,')')
...
c     parameter (ngrid=10512,nuc=15)
      ...
      dimension nh(20),nf(20),nm(20),nn(20),fhr(21),fal(21)
...
c
      ...
      enddo
      fhr(21) = sfnh/(sfnh+sfnm)
      fal(21) = sfnf/(sfnf+sfnn)
      lomin = klmi
      lomax = klmx
c     print 202, fhr
c     print 202, fal
