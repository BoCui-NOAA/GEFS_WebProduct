...
$HOME/evrfy/grads/readin2grads_tg.sh $STYMD $EDYMD $EXPID $FDAYS $SDIR $GDIR

sed -e "s/EXPID/$EXPID/" \
    -e "s/CYMDH/$CYMDH/" \
    $HOME/evrfy/grads/run_4_grads_tg.ctl > pr$EXPID.ctl    
...
sed -e "s/EXPID1/$EXPID1/" \
    -e "s/CSTYMD/$CSTYMD/" \
    -e "s/CEDYMD/$CEDYMD/" \
    $HOME/evrfy/grads/grads_T.gs > map.gs        
#   $HOME/evrfy/grads/grads_T_book.gs > map.gs        

xgrads -cpl "$GDIR/map.gs"

#gxps -c                -i nh500h_tg.gr -o nh500h_tg_$YY$MM.ps  
#gxps -c                -i sh500h_tg.gr -o sh500h_tg_$YY$MM.ps 
...
gxgif -r -x 545 -y 695 -i nh500h_tg.gr   -o nh500h_tg_$gname.gif
#ftpsgi sgi100 put $SGIDIR /ptmp/wx20yz/grads  nh500h_tg_$gname.gif
ftprzdm rzdm put $RZDMDIR /ptmp/wx20yz/grads  nh500h_tg_$gname.gif

gxgif -r -x 545 -y 695 -i sh500h_tg.gr   -o sh500h_tg_$gname.gif
#ftpsgi sgi100 put $SGIDIR /ptmp/wx20yz/grads  sh500h_tg_$gname.gif
ftprzdm rzdm put $RZDMDIR /ptmp/wx20yz/grads  sh500h_tg_$gname.gif
