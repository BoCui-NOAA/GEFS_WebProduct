...
c
      program VRFYEN
      parameter (ihmax=408,imem=23,imemp1=imem+1,imemp5=imem+5)
     ...
     &           '23tsa','00Zap','12Zap','20ems','23tms','Meds '/
      data flab /'T23  ','T+1  ','T1000','T1012','T20  ','T+1  ',
     &           'R23  ','Rs5  ','Sg4  '/
...
c ----
         call talagr(23,10,en,an,it2,ajk,xjk,pjk,jk,101)           
         call talagr(23,10,en,an,it3,ajk,xjk,pjk,jk,102)           
         call talagr(23,20,en,an,it4,ajk,xjk,pjk,jk,0)           
...
c        sprf(kk,jj) = sqrt(sprf(kk,jj)/float(igrp(kk))/acwta(jj))
       ...
       enddo
        ...
        call fptala(fgrid,pgrid,23,fitavg)
c       call fptala(fgrid,pgrid,14,f14avg)
c ----
...
c ---- 5. 5 pairs      12Z scores 
        do jj = 1, 4
         do kk = 1, 3  
          ...
          frms(kk,jj,5) = 0.0 
          do mm = 14, imem
           fcor(kk,jj,5) = fcor(kk,jj,5) + cor(kk,jj,mm)/10.0
           frms(kk,jj,5) = frms(kk,jj,5) + rms(kk,jj,mm)/10.0
          enddo
         enddo
        enddo
       if (ii.gt.1) then
c ---- 3. 5 pairs      00Z scores
        do jj = 1, 4
         ...
         enddo
        enddo
c ----11. 23 members ( totally )
        do jj = 1, 4
         do kk = 1, 3  
          do mm = 1, imem
           fcor(kk,jj,6) = fcor(kk,jj,6) + cor(kk,jj,mm)/float(imem)
           frms(kk,jj,6) = frms(kk,jj,6) + rms(kk,jj,mm)/float(imem)
          enddo
         enddo
        enddo
       endif
...
c ----                
       ...
       do i = 1, 3
        ...
        write(*,212)   flab(5),(fit(j,i,4),j=1,12)
        write(*,212)   flab(5),(fit(j,i,4),j=13,21)
c       write(*,212)   flab(6),(f14avg(j,i),j=1,9)
c       write(*,212)   flab(6),(f14avg(j,i),j=10,15)
        write(*,212)   flab(7),(rel(j,i),j=1,12)
        write(*,212)   flab(7),(rel(j,i),j=13,23)
        ...
        write(81,212)   flab(5),(fit(j,i,4),j=1,12)
        write(81,212)   flab(5),(fit(j,i,4),j=13,21)
c       write(81,212)   flab(6),(f14avg(j,i),j=1,9)
c       write(81,212)   flab(6),(f14avg(j,i),j=10,15)
        write(81,212)   flab(7),(rel(j,i),j=1,12)
        write(81,212)   flab(7),(rel(j,i),j=13,23)
