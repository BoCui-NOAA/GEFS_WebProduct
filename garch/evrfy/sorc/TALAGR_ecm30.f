...
c ----
      if(jcon.eq.1) then         ! for 12z run five pairs
       do i=2,31   
        prms=prms + (ve-en(i))*(ve-en(i))
       enddo
      endif

      if (m.eq.31.and.en(1).eq.-9999.99) goto 999
      if (m.eq.30.and.en(2).eq.-9999.99) goto 999

c ----
c Regrouping the data arrary
c     for m = 11  do nothing
c         m = 10  10 ensemble members      
c ----
      if (m.eq.30) then
       do i=1,30
        en(i)=en(i+1)
