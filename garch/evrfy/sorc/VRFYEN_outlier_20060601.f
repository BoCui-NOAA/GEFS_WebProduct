...
c                     imem   ---> forecasting members ( NCEP are 40 members )
c
c
...
c
      program VRFYEN
      parameter (ihmax=408,imem=40)

      dimension agrid(144,73),ogrid(144,73),fgrid(144,73,imem)
      dimension pgrid(144,73,imem),qgrid(144,73)
      dimension kdate(100)
...
c       ii.ge.4 --> start lead time is 24 hours
       if (ilev.eq.500) then
        if (ii.ge.4.or.ii.lt.16) then
         call outlie_new(agrid,fgrid,pgrid,40,ogrid,qgrid)
c        do lon = 1, 144
c        write (*,'(i3,10(f7.1))') lon,(ogrid(lon,lat),lat=18,27)
c        enddo
         call putgrb(ogrid,ilev,9,jdate,ifhr)
         call putgrb(agrid,ilev,7,ndate,0)
ccc      80 ensemble mean by using Geometric height message
         call putgrb(qgrid,ilev,8,ndate,0)
        endif
       endif
