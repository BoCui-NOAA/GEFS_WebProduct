...
c
      ...
      dimension       spr(4),sprd(4,3),f16avg(17,3),f10avg(11,3)
...
c     seed=kdate(1,1)
      seed=ndate      
      print *, seed
      call durand(seed,16,randm)
      call isortx(randm,1,16,indx)
...
c ----
        ...
        do lon = 1, 144                  ! do loop for each longitude    
         ...
         do im = 2, 17
          en(im) = fgrid(lon,lat,indx(im-1)+1)
         enddo
         an      = agrid(lon,lat)
c ----
c en --> forecast
...
c ----
         call talagr(17,8,en,an,it2,ajk,xjk,pjk,irel,0)           
         call talagr(17,10,en,an,it3,ajk,xjk,pjk,irel,0)           
         call talagr(17,16,en,an,it4,ajk,xjk,pjk,irel,0)           
...
c        elseif ( lat.ge.29.and.lat.le.45 ) then
 ...
 2010 continue
         ...
         enddo
         do kk = 1, 4
           sprd(kk,jj)  = sprd(kk,jj)  + spr(kk)*weight/144.0
         enddo
 2020 continue
...
c        sprd(kk,jj) = sqrt(sprd(kk,jj)/float(igrp(kk))/acwta(jj))
         sprd(kk,jj) = sqrt(sprd(kk,jj)/float(igrp(kk)-1)/acwta(jj))
        enddo
       enddo
...
c ---- 2. 4 pairs a  T00Z scores 
        do jj = 1, 4
         do kk = 1, 3  
          do mm = 2, 9  
c          if (jj.eq.1.and.kk.eq.1) then
c           if (cor(kk,jj,mm).gt.1.1) then
c            print *, "kk,jj,mm=",kk,jj,mm,cor(kk,jj,mm)
c           endif
c          endif
...
c      endif
       do i = 1, 3
        ...
        write(*,212)   flab(9),(sprd(j,i),j=1,4)
        write(81,215)   chem(i)
        ...
        write(81,212)   flab(9),(sprd(j,i),j=1,4)
       enddo
c
