...
c
      ...
      dimension ffgrid(144,73,31),ppgrid(144,73,31)
      dimension dgrid(144,73,7),pgrid(144,73,imem)
      dimension cor(3,4,imemp4),rms(3,4,imemp4),acwt(73)
      dimension fcor(3,4,13),frms(3,4,13),irel(31),rel(imem,3)
      dimension ir10(10),r10(10,3),ir30(30)
      dimension xrms(144,73),rel5(5,3),acwta(3)
      dimension en(31),it(imemp1,4),fit(imemp1,3,4),fitavg(imemp1,3)
      dimension fit10(11,3)
      dimension jk(imem),it1(32),it2(31),ikk(2)
...
c ----
       ...
       call getgrb(ffgrid,cpgbf,cpgif,ifld,ilev,jdate,ifhr,15)
c ----
...
c        do im = 1, imem                 ! 11 ensemble members
         do im = 1, 31                   ! 31 ensemble members
          en(im) = ffgrid(lon,lat,im)
         enddo
         an      = agrid(lon,lat)
c ----
c en --> forecast
...
c ----
c        call talagr(31,10,en,an,it2,ave,xmed10,pijk,ir10,jcon)           
         call talagr(31,30,en,an,it2,ave,xmed30,pijk,ir30,jcon)           
         call talagr(31,31,en,an,it1,ave,xmed31,prms,irel,jcon)
c ----
...
c ----
         ...
         dgrid(lon,lat,2) = xmed31       ! 31 members
         dgrid(lon,lat,4) = xmed30       ! 30 members 

c ----
c spread
c ----
         call spread(31,en,ave,spr2,2)   ! 30 members
         dgrid(lon,lat,3) = ave          ! 30 members
         call spread(31,en,ave,spr1,1)   ! 11 members
...
c      call getgrb(pgrid,cpgbp,cpgip,ifld,ilev,jdat1,ifhr24,10)
c      call fptala(fgrid,pgrid,11,11,fitavg)
c      call fptala(fgrid,pgrid,10,10,fit10)   ! T12Z 10 members
c ----
c to calculate amomaly corrlation and rms error
c ----
       do im=1,imem
        call states(agrid,ffgrid(1,1,im),ifld,ilev,jdate,cor(1,1,im),
