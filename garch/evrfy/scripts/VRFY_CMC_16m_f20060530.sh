...
while [ $kcnt -le $nstep ]; do
   ...
   if [ -s $GLOBAL/ens/z$ilv\_cmc_fnl.$YYYY ]; then
      cp $GLOBAL/ens/z$ilv\_cmc_fnl.$YYYY cmc_anl.$ilv
   fi
...
###
   ...
   cat $GLOBAL/cmc_ens/z$ilv.$stymd\00 >$pgb
   $windex $pgb $pgi
   var="cfiles(1,2)='$pgb',"
   echo "$var"            
   echo "$var" >>input2
   ...
   cat $GLOBAL/cmc_ens/z$ilv.$ymdp24 >$pgb
   $windex $pgb $pgi
   var="cfiles(1,3)='$pgb',"
   echo "$var"            
   echo "$var" >>input2
...
#/nfsuser/g01/wx20yz/evrfy/exec/vrfy_cmc  <input0
#/nfsuser/g01/wx20yz/evrfy/exec/vrfy_cmc_f20060530 <input0
