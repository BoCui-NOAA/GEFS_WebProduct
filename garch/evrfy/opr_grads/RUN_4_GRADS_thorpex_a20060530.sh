...
$HOME/evrfy/opr_grads/readin2grads_a20060530.sh $STYMD $EDYMD $EXPID $FDAYS $SDIR $GDIR $ICNT
...
sed -e "s/EXPID/$EXPID/" \
    -e "s/CYMDH/$CYMDH/" \
    $HOME/evrfy/opr_grads/run_4_gradm_a20060530.ctl > pm$EXPID.ctl    

sed -e "s/EXPID/$EXPID/" \
    -e "s/CYMDH/$CYMDH/" \
    $HOME/evrfy/opr_grads/run_4_gradm_test.ctl > pt$EXPID.ctl    
...
sed -e "s/EXPID1/$EXPID1/" \
    ...
    -e "s/CSTYMD/$CSTYMD/" \
    -e "s/CEDYMD/$CEDYMD/" \
    $HOME/evrfy/opr_grads/grads_linem_test.gs >map.gs

#xgrads -cbl "$GDIR/map.gs"
## to run this map.gs, you need to run data again
...
sed -e "s/EXPID1/$EXPID1/" \
    ...
    -e "s/CSTYMD/$CSTYMD/" \
    -e "s/CEDYMD/$CEDYMD/" \
    $HOME/evrfy/opr_grads/grads_die_15d_test_a20060530.gs >map.gs
...
gxgif -r -x 1100 -y 850 -i nh500h_ac_die.gr -o nh500h_ac_die_$YYYY.gif
ftprzdm rzdm put $RZDMDIR /ptmp/wx20yz/oprm_grads  nh500h_ac_die_$YYYY.gif
