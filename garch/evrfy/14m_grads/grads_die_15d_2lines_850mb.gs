...
'define ta=ave('var4',time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)
'define tb=ave('var4'.2,time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)
'define tc=ave('var1',time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)
...
'set cmark 3'
'd tc'
'draw title 'titreg'850 mb TEMP. ( wave 1-20 ) \ Average For 'ts' - 'te''
...
'run linesmpos.gs leg_pac_3lines 2.3 1.2 3.8 2.2'
...
'define ta=ave('var4'/100,time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'define tb=ave('var4'.2/100,time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'define tc=ave('var1'/100,time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'set y 2'
'define td=ave('var5',time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'define te=ave('var5'.2,time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'set y 3'
'define ti=ave('var3',time='ts',time='te')'
say result
line=sublin(result,1)
word=subwrd(line,4)

'set t  1'
'set vrange 0.0 'vrmax
'set ccolor 1'
'set cmark 1'
'd ta'
'set ccolor 2'
...
'set cmark 5'
'd ti'
'draw title 'titreg' 850 mb TEMP. \ Average For 'ts' - 'te''
'draw xlab Forecast days'
...
'draw string 9.76 0.55 14'
'draw string 10.32 0.55 15'
'run linesmpos.gs leg_rms_2lines 8.6 1.0 10.1 2.2'
...
return str
function getvrmax(reg)
if(reg=1);vrmax=5;endif
if(reg=2);vrmax=5;endif
if(reg=3);vrmax=5;endif
return vrmax
function getreg(reg)
