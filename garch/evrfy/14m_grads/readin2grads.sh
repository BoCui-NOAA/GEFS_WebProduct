...
if [ $# -le 6 ]; then
 echo "Usage: $0 need STYMD,EDYMD,EXP_ID,IFDAYS(max=16),SDIR,PDIR and ILEVEL"
 exit 8
fi
...
while [ $STYMD -le $EDYMD ]; do
 (( icnt += 1 ))
 if [ -s $EXdir/$SNAME$EXPID.$STYMD\00 ]; then
...
echo "ilevel=$ilevel"                               >>input
echo "/" >>input

cat input
rm grads_$EXPID.dat

$SHOME/$LOGNAME/evrfy/14m_grads/14m_grads <input
