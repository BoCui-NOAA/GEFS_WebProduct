...
#   export CDATE=$CDATE; $SHOME/yan.luo/jif_cqpf/grads/PLOT.sh
else
   ### control plots for global, cpc and 6-hour cycle
   ###  0=run, 1=not run
   ICGLB=1
...
cmd=$SHOME/yan.luo/wpqpf/scripts
...
MM=`grep "$mm" $SHOME/yan.luo/bin/mon2mon | cut -c8-10` 
...
cd $SHOME/yan.luo/wpqpf/scripts;pwd
...
tmpdir=$PTMP/yan.luo/6hr_$HCYCLE
mkdir $tmpdir
cd $tmpdir
rm $tmpdir/*
cp $SHOME/yan.luo/grads/rgbset.gs .
cp $SHOME/yan.luo/grads/cbar.gs .
IC=$HCYCLE
...
MON=`grep "$mm" $SHOME/yan.luo/bin/mon2mon | cut -c8-10` 
RZDMDIR=/home/people/emc/www/htdocs/gmb/yluo/pqpf_6h                          
ftpemcrzdmmkdir emcrzdm $RZDMDIR $ymd 
...
ftpemcrzdmrmdir emcrzdm $RZDMDIR $YMDM30
...
do
 ...
 ifile=/lfs/h2/emc/vpppg/noscrub/yan.luo/ecmwf_gefs/noscrub/ENS/$file.${ymd}${IC} 
...
gribmap -i A$amt.t${IC}z.$file.ctl
...
#ftpemcrzdm emcrzdm put $RZDMDIR/$ymd $tmpdir a$amt\_$CDATE\_F$tm1\-$tm2.png

done
scp *.png   wd20yl@emcrzdm:$RZDMDIR/$ymd
...
ftpemcrzdm emcrzdm put $RZDMDIR/$ymd $tmpdir a$amt.t$IC\z.html       
...
ftpemcrzdm emcrzdm put $RZDMDIR $tmpdir PQPF_6h.html  
