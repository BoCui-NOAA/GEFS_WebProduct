...
fdir=/global/pr$pr       
ddir=/nbns/global/wx20bm/ecm_nov2005
fn=pgbf

if [  "$pr" = "s" ]; then
fdir=/nbns/global/wx23bk/gfs
fi
...
#ivrf=0
ivrf=0   ### changed since 07/10/2002 when parellel x move to operational
...
while [ $kcnt -le $istep ]; do
 ...
 for file in GDAS FCST ; do
  ...
  if [ "$file" = "GDAS" -a $ivrf -eq 1 ]; then
   echo "Using ECMWF analysis ++++++"
   cp /global/ecm/ecmgrb.$pymdh fnl.$pymdh
   ...
   if [ -s $DIRandFile ]; then
    PGBLEN=`wgrib -V -ncep_opn $DIRandFile | grep nxny | sed -n '1 p' | awk '{print $9}'`
...
cat scores.out 
cp scores.out /global/vrfy_es/SCORES$pr.$stymd$3

if [ $3 -eq 00 -a "$pr" != "y" ]; then
 cd /global/vrfy
