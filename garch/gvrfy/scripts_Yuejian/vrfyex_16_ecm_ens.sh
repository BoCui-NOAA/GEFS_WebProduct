...
ECMCVT=$SHOME/$LOGNAME/gvrfy/scripts/ecmcvt.sh
EXECS=$SHOME/$LOGNAME/gvrfy/exec/vrfy_0006
...
if [ "$pr" = "e" ]; then
 EXECS=$SHOME/$LOGNAME/gvrfy/exec/vrfy_0110
 fdir=$GLOBAL/ecm_ens         
 ddir=$GLOBAL/ecm_ens   
 fn=ecmgrb
 anl=
...
while [ $kcnt -le $istep ]; do
 ...
 if [ -s $DIRandFile ]; then
  if [ "$pr" = "e" -a "$file" = "FCST" ]; then
   tmpfile=$fdir/z1000.$fymdh 
   wgrib -PDS10 $tmpfile | grep " 1 2 1 255" | grep "mb:${fhr}hr" | cut -c1-80 | wgrib -i $tmpfile -grib -o $Sfile
   tmpfile=$fdir/u850.$fymdh 
   wgrib -PDS10 $tmpfile | grep " 1 2 1 255" | grep "mb:${fhr}hr" | cut -c1-80 | wgrib -i $tmpfile -grib -append -o $Sfile
  ...
  if [ "$pr" = "e" -a "$file" = "GDAS" ]; then
   tmpfile=$ddir/z1000.$pymdh 
   wgrib -PDS10 $tmpfile | grep " 1 2 1 255" | grep "mb:anl" | cut -c1-80 | wgrib -i $tmpfile -grib -o $Sfile
   tmpfile=$ddir/u850.$pymdh 
   wgrib -PDS10 $tmpfile | grep " 1 2 1 255" | grep "mb:anl" | cut -c1-80 | wgrib -i $tmpfile -grib -append -o $Sfile
...
cat scores.out 
cp scores.out $GLOBAL/vrfy/SCORESec.$stymd$3
