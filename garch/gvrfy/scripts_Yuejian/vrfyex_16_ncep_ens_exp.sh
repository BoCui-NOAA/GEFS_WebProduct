...
###
 ...
 fdir=$GLOBAL/ENS             
 ddir=$GLOBAL/prs   
 ddir=$NGLOBAL/wx20yz/GFS_test/T382L64
 else
 fdir=/global/noscrub/wx20yz/ens_gel$2
 ddir=$GLOBAL/ENS   
 fi
 fn=
 anl=
...
#  GDAS) DIRandFile=$ddir/z500.$pymdh;Sfile=pgbanl.$pymdh;FF=cfilea;; 
 ...
 if [ -s $DIRandFile ]; then
  ...
  if [ "$file" = "GDAS" ]; then
  if [ "$pr" = "c" ]; then
  cp $ddir/pgbanl.$pymdh pgbanl.$pymdh
  else
...
cat scores.out 
#cp scores.out $GLOBAL/vrfy/SCORESnc.$stymd$3
cp scores.out $NGLOBAL/wx20yz/tmp/SCORES$2\0.$stymd\00
