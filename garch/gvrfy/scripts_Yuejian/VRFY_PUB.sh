...
vrfy_table=$1
export VFSDAY=$VFSDAY
export VFEDAY=$VFEDAY
...
if [ $IVRFY -eq 1 ]; then
 ...
 if [ $PGBLEN -eq 10512 ]; then
  ...
  echo ""
  $COPYGB -g2 -i1 $ddir/pgbf00.$CDATE pgbf00.$CDATE.index pgbf00.$CDATE
  $windex pgbf00.$CDATE pgbf00.$CDATE.index
  ls -l pgbf00.$CDATE 
 elif [ "$PGBLEN" = "" ]; then
...
cp SCORES$EXP_ID.$CDATE $GLOBAL/vrfy/SCORES$EXP_ID.$CDATE
