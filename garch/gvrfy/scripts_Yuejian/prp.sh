...
fdir=/gloptmp/wx23ph/datprp 
ddir=/gloptmp/wx23ph/datprp 
fn=pgbf
...
while [ $kcnt -le $istep ]; do
 ...
 if [ -s $DIRandFile ]; then
  $windex $DIRandFile $Sfile.index                 
  $COPYGB -g2 $DIRandFile $Sfile.index $Sfile
  rm $Sfile.index 
  ...
  if [ "$file" = "GDAS" -a -s /global/prx/pgbf00.$pymdh ]; then
   echo " Using /global/prx/pgbf00.$pymdh instead of exp. $pr "
   cp /global/prx/pgbf00.$pymdh $Sfile
   $windex $Sfile $Sfile.index
   var=" $FF(1,$kcnt)='$Sfile',$FF(2,$kcnt)='$Sfile.index',"
   echo "$var" 
   echo "$var" >>input2
...
cat scores.out 
cp scores.out /global/prp/SCORES$pr.$stymd
stymd=`expr $stymd + 1`
