...
fdir=/global/help/zou/$pr       
ddir=/global/help/zou/gps       
fn=pgbf
...
cat <<paramEOF >input1
 ...
 /    
 &namin  
 ihem=1,la1=6,la2=29,lo1=1,lo2=144,l1=24,l2=36,nhrs=$nh,nvhrs=$ih,
...
while [ $kcnt -le $istep ]; do
 ...
 if [ "$file" = "GDAS" -a $ivrf -eq 1 ]; then
   ...
   getgrib z1000_ncep_fnl.$YEAR z1000.$pymdh $pymdh 00 7 1000
   getgrib z500_ncep_fnl.$YEAR z500.$pymdh $pymdh 00 7 500
   cat $tmpdir/z1000.$pymdh $tmpdir/z500.$pymdh >pgbanl.$pymdh
   $windex pgbanl.$pymdh pgbanl.$pymdh.index
   fi
   var=" $FF(1,$kcnt)='pgbanl.$pymdh',$FF(2,$kcnt)='pgbanl.$pymdh.index',"
   echo "$var" 
   echo "$var" >>input2
 else
...
cat scores.out 
cp scores.out /global/help/zou/results4/SCORES$pr.$stymd
stymd=`expr $stymd + 1`
