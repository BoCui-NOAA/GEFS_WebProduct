...
ddir=/ptmp/wx20yz/xanl    
3ddir=/ptmp/wx20mi/xanl    
fn=pgbf
...
while [ $kcnt -le $istep ]; do
 ...
 do 
  ...
  case $file in
   ...
   #GDAS) DIRandFile=$ddir/pgbanl.$pymd$AHR;Sfile=$fn$anl.$pymd$AHR;FF=cfilea;; 
...
cat scores.out 
cp scores.out /global/help/wx20mi/SCORESh.$fymd  
