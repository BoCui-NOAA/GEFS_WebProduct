...
mkdir $PTMP/$LOGNAME/tpry2q_pry
cd    $PTMP/$LOGNAME/tpry2q_pry
...
cp $GLOBAL/pry/pgbf$FHR.$ISTYMDH pgbf$FHR.$ISTYMDH            
...
$SHOME/$LOGNAME/gvrfy/sorc/tpry2q_gfs.x <input
if [ $FHR -eq 00 ]; then
cp $qfile $GLOBAL/pryq/pgbanl.$ISTYMDH
else
cp $qfile $GLOBAL/pryq/pgbf$FHR.$ISTYMDH
fi
