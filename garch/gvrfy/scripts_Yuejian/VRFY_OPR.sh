...
nh=24       ### nh=120  ( verifying upto 120 hours forecasts )
ih=12       ### ih=12   ( forecast interval )
ictl=0      ### ictl=0  ( against one analysis ) or ictl=1 ( against one initial )
iclim=0     ### iclim=0 ( CAC Climatology ) and iclim=1 ( CDAS/ReAnl Climatology)
...
if [ $iclim -eq 0 ]; then
 ...
 cp $CLI23Y/pgb.f12${m1}.00Z          .
 cp $CLI23Y/pgb.f12${m1}.00Z.index    .
 cp $CLI23Y/pgb.f00${m2}.00Z          .
 cp $CLI23Y/pgb.f00${m2}.00Z.index    .
 cp $CLI23Y/pgb.f12${m2}.00Z          .
 cp $CLI23Y/pgb.f12${m2}.00Z.index    .
 cp $CLI23Y/pgb.f00${m3}.00Z          .
 cp $CLI23Y/pgb.f00${m3}.00Z.index    .
 cp $CLI23Y/pgb.f12${m3}.00Z          .
 cp $CLI23Y/pgb.f12${m3}.00Z.index    .
fi
