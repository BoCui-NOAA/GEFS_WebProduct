...
export IVRFY=1
export EXP_ID=lo
export DIR_ANL=/gpfsuser/emc3/prylo
export DIR_FST=/gpfsuser/emc3/prylo
export FHOURS=384
...
if [ $IVRFY -eq 1 ]; then
 ...
 if [ ! -s $ddir/pgbf00.fnl.$CDATE ]; then
  echo "file $ddir/pgbf00.fnl.$CDATE does not exist, quit!!!"
  exit 16
 fi
 PGBLEN=`wgrib -V -ncep_opn $ddir/pgbf00.fnl.$CDATE | grep nxny | sed -n '1 p' | awk '{print $9}'`
 echo " $ddir/pgbf00.fnl.$CDATE has length of $PGBLEN "
 if [ $PGBLEN -eq 10512 ]; then
  cp $ddir/pgbf00.fnl.$CDATE pgbf00.$CDATE
  $windex pgbf00.$CDATE pgbf00.$CDATE.index
 elif [ $PGBLEN -eq 65160 ]; then
  echo " Reduceing resolution from 1*1 to 2.5*2.5 "
  echo ""
  $COPYGB -g2 -x $ddir/pgbf00.fnl.$CDATE pgbf00.$CDATE
  $windex pgbf00.$CDATE pgbf00.$CDATE.index
  ls -l pgbf00.$CDATE 
 elif [ "$PGBLEN" = "" ]; then
...
while [ $istep -lt $nstep ]; do
 ...
 if [ ! -s $fdir/pgbf$ih.gfs.$YMDH ]; then
  echo "file $fdir/pgbf$ih.gfs.$YMDH does not exist, caution!!!"
  echo ""
 else
 PGBLEN=`wgrib -V -ncep_opn $fdir/pgbf$ih.gfs.$YMDH | grep nxny | sed -n '1 p' | awk '{print $9}'`
 echo " $fdir/pgbf$ih.gfs.$YMDH has length of $PGBLEN "
 echo ""
 cp $fdir/pgbf$ih.gfs.$YMDH pgbf$ih.$YMDH
 ...
 if [ $PGBLEN -eq 10512 ]; then
  ...
  echo " Reduceing resolution from 1*1 to 2.5*2.5 "
  echo "" 
  $COPYGB -g2 -x $fdir/pgbf$ih.gfs.$YMDH pgbf$ih.$YMDH
  $windex pgbf$ih.$YMDH pgbf$ih.$YMDH.index
  ls -l pgbf$ih.$YMDH
 elif [ "$PGBLEN" = "" ]; then
...
#cp SCORES$EXP_ID.$CDATE /global/exp/pry
