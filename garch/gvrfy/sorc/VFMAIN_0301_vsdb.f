...
c              ACGRIDV---> to calaulate wind vector AC using grid values
...
c     add vsdb output                by Yuejian Zhu (07/20/05)
c
C--------+---------+---------+---------+---------+----------+---------+--
      program VFMAIN
      parameter (idmax=100)
      ...
      dimension ffmn(20,73),famn(20,73)
      dimension fnum(20,73),dnm1(20,73),dnm2(20,73)
      dimension q1(4,73),q2(4,73),q3(4,73),q4(4,73),q5(4,73)
      dimension w1(73),w2(73),w3(73),w4(73),w5(73)
      dimension qx1(4),qx2(4),qx3(4),qx4(4),qx5(4),pac(4),weight(73)
      dimension ac1(idmax),ac2(idmax),ac3(idmax),ac4(idmax)
      dimension rms(idmax),erm(idmax),rmc(idmax),erc(idmax)
      dimension iymdh(idmax,5),ireg(4)
      character*132 vdbhdr132
      character*10 chd(9),chrout
      character*5 chem(4)
      character*7 lhem(4)
      character*3 cfld(200)
      character*4 chfld(200)
      character*8 ctype(200,2)
     ...
     .                iclim,ic
      ...
      open(unit=52,file='vsdb.out',err=1020)
c
...
c....  when ifd.eq.102, considering wind vector    
        do i = 1, 144
         do j = 1, 73
...
c....
       if (ic.eq.1) then      
        ...
        do i = 1, 20
         do l = 1, 73
          ffmn(i,l)  =(af(i,l)-ac(i,l)) + (bf(i,l)-bc(i,l))
          famn(i,l)  =(ao(i,l)-ac(i,l)) + (bo(i,l)-bc(i,l))
     ...
     .                (bf(i,l)-bc(i,l))*(bo(i,l)-bc(i,l))
        ...
        do l = 1, 73
         ...
         call sumwav(ffmn(1,l),q4(1,l))
         call sumwav(famn(1,l),q5(1,l))
        enddo    
c.....  avg over lat band l1-l2, from north pole to south pole 
        do j = 1, 4
         ...
         call latwmn(w4,l1,l2,qx4(j))
         call latwmn(w5,l1,l2,qx5(j))
        enddo    
c.....  combine terms to get anom corr for this day
        count=10512.0
        kymdh=iymdh(iday,5)*1000000 + iymdh(iday,3)*10000
     &     +  iymdh(iday,4)*100     + iymdh(iday,2)
        kymdh=2000000000 + jdata
        do j = 1, 4
         pac(j)=qx1(j)/(sqrt(qx2(j))*sqrt(qx3(j)))

         chd(1) = 'V01'
         chd(2) = 'GFS'
         call cvntoc(iymdh(iday,1),chrout)
         chd(3) = chrout
         call cvntoc(kymdh,chrout)
         chd(4) = chrout
         chd(5) = 'GLBANL'
         call cvntoc(ireg(ihem),chrout)
         chd(6) = chrout
         chd(7) = ctype(ifd,1)
         chd(8) = chfld(ifd)
         call cvntoc(ilv,chrout)
         write(chd(9),809) chrout
         call vdbhdr(vdbhdr132,iend,chd)
c        write (*,910)  vdbhdr132(:iend),count,
c    &                  qx4(j),qx5(j),qx1(j),qx2(j),qx3(j)
         write (52,910) vdbhdr132(:iend),count,
     &                  qx4(j),qx5(j),qx1(j),qx2(j),qx3(j)

        enddo    
       else
        call acgrid(fanl,fcst,clim,weight,la1,la2,lo1,lo2,pac)
       endif
c....
       if (ifd.eq.102) then
        call acgridv(uanl,vanl,ufst,vfst,ctmp,clit,
     *               weight,la1,la2,lo1,lo2,pac)
        ac1 (iday) = 0.0
        ac2 (iday) = 0.0
        ac3 (iday) = 0.0
        ac4 (iday) = pac(4)
...
C--------+---------+---------+---------+---------+----------+---------+--
 ...
 910  format (a,f7.0,5e18.9)
 920  format (a,f7.0,7e18.9)
 1020 continue
      stop    
      end
