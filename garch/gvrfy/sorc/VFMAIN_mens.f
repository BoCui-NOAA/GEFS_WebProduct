...
C--------+---------+---------+---------+---------+----------+---------+--
      ...
      dimension fcst1(144,73),fcst2(144,73)
      dimension ufst1(144,73),ufst2(144,73)
      dimension vfst1(144,73),vfst2(144,73)
      ...
      character*80 cfilea(2,100),cfilef(4,100),ca1,ca2,cf1,cf2,cf3,cf4 
...
c
       ...
       if (ifd.gt.100) then
        call getanl(ufst1,cf1,cf2,33,ilv,ndatef,ifh)
        call getanl(vfst1,cf1,cf2,34,ilv,ndatef,ifh)
        call getanl(ufst2,cf3,cf4,33,ilv,ndatef,ifh)
        call getanl(vfst2,cf3,cf4,34,ilv,ndatef,ifh)
        if(ufst1(1,1).eq.-9999.99) goto 998
        if(vfst1(1,1).eq.-9999.99) goto 998
        if(ufst2(1,1).eq.-9999.99) goto 998
        if(vfst2(1,1).eq.-9999.99) goto 998
        do lat = 1, 73
         do lon = 1, 144
          ufst(lon,lat) = (ufst1(lon,lat)+ufst2(lon,lat))*0.5
          vfst(lon,lat) = (vfst1(lon,lat)+vfst2(lon,lat))*0.5
         enddo
        enddo
        call fitr30(ufst)
        call fitr30(vfst)
       else
        call getanl(fcst1,cf1,cf2,ifd,ilv,ndatef,ifh)
        call getanl(fcst2,cf3,cf4,ifd,ilv,ndatef,ifh)
        do lat = 1, 73
         do lon = 1, 144
          fcst(lon,lat) = (fcst1(lon,lat)+fcst2(lon,lat))*0.5
         enddo
        enddo
        if(fcst1(1,1).eq.-9999.99.or.fcst2(1,1).eq.-9999.99) goto 998
