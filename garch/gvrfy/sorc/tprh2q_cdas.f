...
c
c      call baopenr(12,indfile,ierr)
c        if(ierr.ne.0) then
c        print *,'error opening file ',indfile
c            stop
c      endif
c
...
c..   compute q now..in g/Kg
       ...
       if(RH .gt. 0.) then
       Q=TPRH2Q(T,P,RH)
ccc    changed by 03/28/2005 (for kg/kg)
c      gridq(i,j)=Q*1000
       gridq(i,j)=Q
       endif
c      print *, 'rh=',rh*100.,' t=',t,' p=',p,' q=',q*1000.
       enddo
       enddo
c
       if(idbug.eq.1) then
...
c       wne: pole problem fix
      ...
      W=W+DTH
10    CONTINUE
C     print *, "J=",J," X,W,X/W,DTH=",X,W,X/W,DTH
11    CONTINUE
      GRIDV=X/W
      RETURN
      END
