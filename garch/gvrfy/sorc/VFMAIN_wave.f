...
c     add wave 21-35, 36-60          by Yuejian Zhu (10/10/01)
c
C--------+---------+---------+---------+---------+----------+---------+--
      program VFMAIN
      parameter (idmax=25)
      dimension fcst(144,73),fanl(144,73),clim(144,73)
      dimension dfst(144,73),danl(144,73),dclm(144,73)
      ...
      dimension qx1(4),qx2(4),qx3(4),qx4(4),qxy(4),weight(73)
      dimension ac1(idmax),ac2(idmax),ac3(idmax),ac4(idmax)
      dimension ac5(idmax),ac6(idmax)
...
c       print *, 'j=',j,' weight=',weight(j)
c       print *, 'j=',lat,' weight=',weight(lat)
      enddo    
c
...
c.... Using CDAS/Reanalysis daily climatology
      if (ifd.eq.7.and.ilv.eq.500) then
       call getdcl(clim,ifd,ilv,ndatea)
      endif
c....
      if (ic.eq.1) then      
      do lat = 1, 73
...
c.....using space transform to regroup
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,1,3,0)
        call fitwav(dfst,1,3,0)
        call fitwav(dclm,1,3,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        qx4(1)=qxy(4)
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,4,9,0)
        call fitwav(dfst,4,9,0)
        call fitwav(dclm,4,9,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        qx4(2)=qxy(4)
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,10,20,0)
        call fitwav(dfst,10,20,0)
        call fitwav(dclm,10,20,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        qx4(3)=qxy(4)
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,1,20,0)
        call fitwav(dfst,1,20,0)
        call fitwav(dclm,1,20,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        qx4(4)=qxy(4)
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,21,30,0)
        call fitwav(dfst,21,30,0)
        call fitwav(dclm,21,30,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        ac5(iday)=qxy(4)
        dfst=fcst
        danl=fanl
        dclm=clim
        call fitwav(danl,31,48,0)
        call fitwav(dfst,31,48,0)
        call fitwav(dclm,31,48,0)
        call acgrid(danl,dfst,dclm,weight,la1,la2,lo1,lo2,qxy)
        ac6(iday)=qxy(4)
      endif
      if (ifd.eq.102) then
...
c
     ...
     &               ac1(i),ac2(i),ac3(i),ac4(i),ac5(i),ac6(i)
      enddo
 ...
 901  format (/,a5,' AC by Waves ( ',a3,' ',a7,' )  -       1-3',
     &              '     4-9    10-20    1-20 ')
 902  format (2x,i3,1x,'hrs after =',i2,'Z ',3i3,i5,'mb',3x,6(f7.4,1x))
C--------+---------+---------+---------+---------+----------+---------+--
