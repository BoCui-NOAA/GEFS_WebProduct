...
c     add wind vector AC calculation by Yuejian Zhu (06/20/00)
c     avilable levels increased      by Yuejian Zhu (10/01/02)
c     multi-time intervel available  by Yuejian Zhu (01/15/03)
c
C--------+---------+---------+---------+---------+----------+---------+--
      program VFMAIN
      parameter (idmax=100)
      dimension fcst(144,73),fanl(144,73),clim(144,73)
      dimension tfcst(144,73),tfanl(144,73),tclim(144,73)
      dimension ufst(144,73),uanl(144,73),uclm(144,73)
      dimension vfst(144,73),vanl(144,73),vclm(144,73)
      dimension fstt(144,73),fant(144,73),clit(144,73),ctmp(144,73)
      ...
      dimension qx1(4),qx2(4),qx3(4),qx4(4),tqx4(4),weight(73)
      ...
      integer ihem,la1,la2,lo1,lo2,l1,l2,nhrs,nvhrs,ifd,ilv,iclim,ic
      character*5 chem(4)
      character*7 lhem(4)
      character*3 cfld(200)
      character*80 cfilea(2,100),cfilef(2,100),ca1,ca2,cf1,cf2 
      namelist/files/ cfilea,cfilef
      namelist/namin/ ihem,la1,la2,lo1,lo2,l1,l2,nhrs,nvhrs,ifd,ilv,
     .                iclim,ic
...
c
      do iday = 1,len
c
c     must use standard file name:
c      pgbf00.2000010100
c      ....
c      pgbf120.2000010100
c
       ca1 = cfilea(1,iday)
       ca2 = cfilea(2,iday)
       cf1 = cfilef(1,iday)
...
c
       ...
       if (ifd.gt.100) then
        ...
        if(ifd.eq.33.or.ifd.eq.34) call fitr30(fcst)
       endif
c
...
c
       ...
       if (ifd.gt.100) then
        call getanl(uanl,ca1,ca2,33,ilv,ndatea,ifha)
        call getanl(vanl,ca1,ca2,34,ilv,ndatea,ifha)
        if(uanl(1,1).eq.-9999.99) goto 998
        if(vanl(1,1).eq.-9999.99) goto 998
        call fitr30(uanl)
        call fitr30(vanl)
       else
        call getanl(fanl,ca1,ca2,ifd,ilv,ndatea,ifha)
        if(fanl(1,1).eq.-9999.99) goto 998
        if(ifd.eq.33.or.ifd.eq.34) call fitr30(fanl)
       endif
c
c --- get the climatology
c
      call iaddate(ndatea,ifha,jdata)
      print *,'ndatea=',ndatea,' ifha=',ifha,' jdata=',jdata
      ndatea=jdata
      ndate=ndatea/100
      jyy = ndate/10000
      jmm = (ndate-jyy*10000)/100
...
c...  Using CPC/Climatology ( if iclim=0 )
       print *, '=== Read CPC climatological data ==='
       call getcac(clim1,ifd,ilv,im1)
       call getcac(clim2,ifd,ilv,im2)
      else
c...  Using CDAS/Reanl 23 years climatology ( grib file )
       print *, '=== Read 23 years CDAS/Reanl Climatological data ==='
       if (ifd.gt.100) then
        ...
        if (ifd.eq.33.or.ifd.eq.34) call fitr30(clim1)
        if (ifd.eq.33.or.ifd.eq.34) call fitr30(clim2)
       endif
      endif
      ...
      elseif (ifd.eq.102) then
       do i = 1, 144
        do j = 1, 73
         ...
         fcst(i,j) = sqrt(uerr*uerr+verr*verr)
         uerr      = uanl(i,j) - clim(i,j)
         verr      = vanl(i,j) - clit(i,j)
c        uerr      = clim(i,j) - ufst(i,j)
c        verr      = clit(i,j) - vfst(i,j)
         ctmp(i,j) = clim(i,j)
...
c
       ...
       tclim = clim
c      do lat = 1, 73
c       call avef(tfcst(1,lat),144,1,1,3)
c       call avef(tfanl(1,lat),144,1,1,3)
c       call avef(tclim(1,lat),144,1,1,3)
c      enddo    
c      call avef(tfcst,144,73,1,3)
c      call avef(tfanl,144,73,1,3)
c      call avef(tclim,144,73,1,3)
       call fitwav_2d(tfcst,144,73,1,3,0)
       call fitwav_2d(tfanl,144,73,1,3,0)
       call fitwav_2d(tclim,144,73,1,3,0)
       zfcst=0.0
       zfanl=0.0
       zclim=0.0
       do lon = 1, 144
        zfcst = zfcst + tfcst(lon,20)/144.0
        zfanl = zfanl + tfanl(lon,20)/144.0
        zclim = zclim + tclim(lon,20)/144.0
       enddo
       write (*,'(4f14.7)') tfcst(1,20),zfcst,zfanl,zclim
       call acgrid(tfanl,tfcst,tclim,weight,la1,la2,lo1,lo2,tqx4)
c      call acgrid(tfanl,tfcst,tclim,weight,l1,l2,lo1,lo2,tqx4)
       qx4(1)=tqx4(1)
c
       tfcst = fcst
       tfanl = fanl
       tclim = clim
c      do lat = 1, 73
c       call avef(tfcst(1,lat),144,1,4,9)
c       call avef(tfanl(1,lat),144,1,4,9)
c       call avef(tclim(1,lat),144,1,4,9)
c      enddo    
c      call avef(tfcst,144,73,4,9)
c      call avef(tfanl,144,73,4,9)
c      call avef(tclim,144,73,4,9)
       call fitwav_2d(tfcst,144,73,4,9,0)
       call fitwav_2d(tfanl,144,73,4,9,0)
       call fitwav_2d(tclim,144,73,4,9,0)
       zfcst=0.0
       zfanl=0.0
       zclim=0.0
       do lon = 1, 144
...
c
       ...
       tclim = clim
c      do lat = 1, 73
c       call avef(tfcst(1,lat),144,1,10,20)
c       call avef(tfanl(1,lat),144,1,10,20)
c       call avef(tclim(1,lat),144,1,10,20)
c      enddo    
c      call avef(tfcst,144,73,10,20)
c      call avef(tfanl,144,73,10,20)
c      call avef(tclim,144,73,10,20)
       call fitwav_2d(tfcst,144,73,10,20,0)
       call fitwav_2d(tfanl,144,73,10,20,0)
       call fitwav_2d(tclim,144,73,10,20,0)
       zfcst=0.0
       zfanl=0.0
       zclim=0.0
       do lon = 1, 144
...
c
       ...
       tclim = clim
c      do lat = 1, 73
c       call avef(tfcst(1,lat),144,1,1,20)
c       call avef(tfanl(1,lat),144,1,1,20)
c       call avef(tclim(1,lat),144,1,1,20)
c      enddo    
c      call avef(tfcst,144,73,1,20)
c      call avef(tfanl,144,73,1,20)
c      call avef(tclim,144,73,1,20)
       call fitwav_2d(tfcst,144,73,1,20,0)
       call fitwav_2d(tfanl,144,73,1,20,0)
       call fitwav_2d(tclim,144,73,1,20,0)
       zfcst=0.0
       zfanl=0.0
       zclim=0.0
       do lon = 1, 144
       ...
       enddo
       write (*,'(4f14.7)') tfcst(1,20),zfcst,zfanl,zclim
c      call acgrid(tfanl,tfcst,tclim,weight,la1,la2,lo1,lo2,tqx4)
       call acgrid(fanl,fcst,clim,weight,la1,la2,lo1,lo2,tqx4)
c      call acgrid(tfanl,tfcst,tclim,weight,l1,l2,lo1,lo2,tqx4)
       qx4(4)=tqx4(4)
      ...
      if (ifd.eq.102) then
       call acgridv(uanl,vanl,ufst,vfst,ctmp,clit,
     *              weight,la1,la2,lo1,lo2,qx4)
       ac1 (iday) = 0.0
       ac2 (iday) = 0.0
       ac3 (iday) = 0.0
       ac4 (iday) = qx4(4)
...
c
     ...
     &               rms(i),erm(i),rmc(i),erc(i)
      enddo
      goto 1000
 901  format (/,a5,' AC by Waves ( ',a3,' ',a7,' )  -       1-3',
     &              '     4-9    10-20    1-20 ')
 902  format (2x,i3,1x,'hrs after =',i2,'Z ',3i3,i5,'mb',3x,4(f7.4,1x))
C--------+---------+---------+---------+---------+----------+---------+--
 903  format (/,a5,' Differences ( ',a3,' ',a7,' )  -   (F-A)',
