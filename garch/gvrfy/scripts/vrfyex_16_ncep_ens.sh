...
dat=/ensemble/save/emc.enspara/gvrfy/data
EXECS=/ensemble/save/emc.enspara/gvrfy/exec/vrfy_0006
...
if [ "$pr" = "c" ]; then
 EXECS=/ensemble/save/emc.enspara/gvrfy/exec/vrfy_0110
 fdir=/ensemble/noscrub/Yan.Luo/REFCST_CTL             
 ddir=/ensemble/noscrub/Yan.Luo/GDAS_ANL   
 fn=
 anl=
 fct=
 HOUR=00
...
while [ $kcnt -le $istep ]; do
 ...
 for file in GDAS FCST ; do
  case $file in
   GDAS) DIRandFile=$ddir/z500_ncep_fnl.$YEAR;Sfile=pgbanl.$pymdh;FF=cfilea;; 
   FCST) DIRandFile=$fdir/z500.$fymdh;Sfile=pgbf$fhr.$fymdh;FF=cfilef;; 
  esac

 if [ -s $DIRandFile ]; then
  if [ "$pr" = "c" -a "$file" = "FCST" ]; then
  tmpfile=$fdir/z1000.$fymdh 
  wgrib $tmpfile | grep "mb:${fhr}hr fcst:" | wgrib -i $tmpfile -grib -o $Sfile
  tmpfile=$fdir/u850.$fymdh 
  wgrib $tmpfile | grep "mb:${fhr}hr fcst:" | wgrib -i $tmpfile -grib -append -o $Sfile
  ...
  if [ "$pr" = "c" -a "$file" = "GDAS" ]; then
  tmpfile=$ddir/z1000_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -o $Sfile
  tmpfile=$ddir/u850_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$ddir/v850_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$ddir/z500_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$ddir/u200_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$ddir/v200_ncep_fnl.$YEAR 
  wgrib $tmpfile | grep $npymdh | wgrib -i $tmpfile -grib -append -o $Sfile
  fi
  if [ $fhr -eq 00 ]; then
  tmpfile=$fdir/z1000.$fymdh
  wgrib $tmpfile | grep "mb:anl:" | wgrib -i $tmpfile -grib -o $Sfile
  tmpfile=$fdir/u850.$fymdh
  wgrib $tmpfile | grep "mb:anl:" | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$fdir/v850.$fymdh
  wgrib $tmpfile | grep "mb:anl:" | wgrib -i $tmpfile -grib -append -o $Sfile
  tmpfile=$fdir/z500.$fymdh
  wgrib $tmpfile | grep "mb:anl:" | wgrib -i $tmpfile -grib -append -o $Sfile
...
cat scores.out 
cp scores.out /ensemble/noscrub/Yan.Luo/vrfy/SCORESnc.$stymd$3
