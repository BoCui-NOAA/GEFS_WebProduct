...
if [ "$pr" = "cfsx" ]; then
 fdir=/ensemble/noscrub/Yan.Luo/ENS_CDAS             
...
while [ $kcnt -le $istep ]; do
 ...
 for file in GDAS FCST ; do
  case $file in
   GDAS) DIRandFile=$ddir/pgbanl.cfsr.$pymdh;Sfile=pgbanl.$pymdh;FF=cfilea;; 
   FCST) DIRandFile=$fdir/z500.$fymdh;Sfile=pgbf$fhr.$fymdh;FF=cfilef;; 
  esac

 if [ -s $DIRandFile ]; then
  if [ "$pr" = "cfsx" -a "$file" = "FCST" ]; then
