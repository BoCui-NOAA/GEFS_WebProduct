...
case $ICNT in
  ...
  2) EXPID2=$EXPID;SDIR=${SDIR_2};GEXPID2=ECM;VHOUR=00;;
  3) EXPID3=$EXPID;SDIR=${SDIR_3};GEXPID3=UKM;VHOUR=00;;
  4) EXPID4=$EXPID;SDIR=${SDIR_4};GEXPID4=gfs;VHOUR=00;;
  5) EXPID5=$EXPID;SDIR=${SDIR_5};GEXPID5=ecm;VHOUR=00;;
  6) EXPID6=$EXPID;SDIR=${SDIR_6};GEXPID6=ukm;VHOUR=00;;
esac

SYMDH=$STYMD$VHOUR

FDAYS=`$SHOME/$LOGNAME/bin/LCHECK.sh $SDIR/SCORES${EXPID}.$SYMDH `
...
$SHOME/$LOGNAME/gvrfy/grads/readin2grads.sh $STYMD $EDYMD $EXPID $FDAYS $SDIR $GDIR 

sed -e "s/EXPID/$EXPID/" \
    -e "s/CYMDH/$CYMDH/" \
    $SHOME/$LOGNAME/gvrfy/grads/run_4_grads.ctl > pr$EXPID.ctl    

done

echo $EXPID1 $EXPID2 $EXPID3 $EXPID4 $EXPID5 $EXPID6

export GDIR=$GDIR;$SHOME/$LOGNAME/gvrfy/grads/GSLINES.sh $ICNT $PFDAYS
...
sed -e "s/ICNT/$ICNT/" \
    ...
    -e "s/PR5/$GEXPID5/" \
    -e "s/PR6/$GEXPID6/" \
    /global/save/wx20yz/vrfy_grads/pub/leg_${ICNT}lines >leg_lines
...
RZDMDIR=/home/people/emc/www/htdocs/gmb/yzhu/GFS_VRFY04
...
ftpemcrzdm emcrzdm put $RZDMDIR $GDIR $fname\_ac.gif
gxgif -r -x 1100 -y 850 -i $fname\_rms.gr -o $fname\_rms.gif
ftpemcrzdm emcrzdm put $RZDMDIR $GDIR $fname\_rms.gif
done
