...
### 
### Plots will be in /stmp/${uname}/grads directory
...
#set -x
GDIR=/stmp/${uname}/grads_VRFY_resolution   
mkdir -p $GDIR
cp $grads_table $GDIR/grads_table.sh
cp $HOME/grads/linesmpos.gs $GDIR
...
case $ICNT in
  1) EXPID1=$EXPID;SDIR=${SDIR_1};GEXPID1=12628;;
  2) EXPID2=$EXPID;SDIR=${SDIR_2};GEXPID2=12664;;
  3) EXPID3=$EXPID;SDIR=${SDIR_3};GEXPID3=12664n;;
  4) EXPID4=$EXPID;SDIR=${SDIR_4};GEXPID4=19028;;
  5) EXPID5=$EXPID;SDIR=${SDIR_5};GEXPID5=126c;;
...
sed -e "s/ICNT/$ICNT/" \
    ...
    -e "s/PR5/$GEXPID5/" \
    -e "s/PR6/$GEXPID6/" \
    $HOME/vrfy_grads/pub/leg_${ICNT}lines >leg_lines
...
RZDMDIR=/home/people/emc/www/htdocs/gmb/yzhu/exp/$EXP\_$YYYYMM
...
sed -e "s/EXP/$EXP/" \
    -e "s/YYYYMM/$YYYYMM/" \
    $HOME/gvrfy/grads/RESOLUTION.HTML >VRFY_daily.html 

scp VRFY_daily.html wd20yz@rzdm:$RZDMDIR
